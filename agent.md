...
- Rate limiting is handled by `logic/dialer.py` (per-line concurrency, per-minute, per-day) plus optional global caps `MAX_CONCURRENT_OUTBOUND_CALLS` / `MAX_CONCURRENT_INBOUND_CALLS` (0 disables). Global throttle is 3 originates/sec. Panel `call_allowed` gates outbound; `STATIC_CONTACTS` is used when panel is disabled. Vira balance errors pause the dialer and set panel to disallow until credit returns.
